...
<section class="section bg-gray">
  ...
  @foreach ($event as $item)
      @if (\Carbon\Carbon::parse($item->date)->timestamp >= \Carbon\Carbon::now()->timestamp)
          <!-- event -->
          <div class="col-lg-4 col-sm-6 mb-5 mb-lg-0">
            <div class="card border-0 rounded-0 hover-shadow">
              ...
              <div class="card-body">
                <!-- location -->
                <p><i class="ti-location-pin text-primary mr-2"></i>{{$item->lieu}}, {{$item->heure}}</p>
                <a href="event-single.html"><h5 class="card-title">{{$item->title}}</h5></a>
...
<section class="section pt-0">
  ...
  <article class="col-lg-4 col-sm-6 mb-5 mb-lg-0">
    <div class="card rounded-0 border-bottom border-primary border-top-0 border-left-0 border-right-0 hover-shadow">
      <img class="card-img-top rounded-0" width="300" height="250" src=" {{asset('storage/'.$item->image)}} " alt="Post thumb">
      <div class="card-body">
        <!-- post meta -->
        <ul class="list-inline mb-3">
          <!-- post date -->
          <li class="list-inline-item mr-3 ml-0">{{$item->created_at}}</li>
          <!-- author -->
          <li class="list-inline-item mr-3 ml-0">By {{$item->auteur}}</li>
        </ul>
        <a href="{{route('journal.detail', [ 'id' => $item->id])}}">
          <h4 class="card-title"> {{$item->title}} </h4>
        </a>
        <p class="card-text"> {{$item->subtitle}} </p>
        <a href="{{route('journal.detail', [ 'id' => $item->id])}}" class="btn btn-primary btn-sm">continuer a lire ...</a>
      </div>
    </div>
  </article>
  @endforeach
